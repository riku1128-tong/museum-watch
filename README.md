...
- 行きたい: カードの「☆ 行きたい」で美術館ごとにお気に入り登録（ピンクの四角いスタンプが付きます）。「行った」とはどちらか片方だけで、行ったら自動で外れます。「行きたい館」だけに絞り込めます
- 行き方: カードをダブルクリック（スマホは「行き方」）で最寄駅と、設定した出発地からの Google マップ経路（電車）を表示
- 並び順: 出発地から近い順（最初の並び順）・閉館が遅い順・会期末が近い順・料金が安い順（出発地の位置は HeartRails Express の駅データか国土地理院の住所検索で調べる）
- よく見る地域: 設定で決めた地域で絞り込んだ状態でページが開きます
- 表示: 右上の歯車 →「表示」で、自動（本体の設定に合わせる）・ライト・ダークを切り替え（この端末に保存）
- 同期: 設定画面でメールアドレスを入れ、届いたコードでログインすると、訪問記録・行きたい館・出発地を PC とスマホで同期（Supabase。テーブル定義は [supabase/museum_watch.sql](supabase/museum_watch.sql)）

- 館・展覧会・都道府県ごとのページ: `m/<館id>.html`・`e/<展覧会id>.html`・`p/<都道府県コード>.html`（検索から見つけてもらうための静的ページ。sitemap.xml 付き）
