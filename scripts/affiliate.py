...
def fill(template: str, m: dict, e: dict | None = None) -> str:
    values = {"name": m["name"], "prefecture": m["prefecture"],
              "municipality": m.get("municipality") or m["prefecture"],
...
def links_html(placement: str, m: dict, e: dict | None = None) -> str:
    ...
    if not ps:
        return ""
    links = "".join(
        f'<a class="aff btn ghost" href="{escape(fill(p["url"], m, e))}" target="_blank" rel="sponsored noopener" '
